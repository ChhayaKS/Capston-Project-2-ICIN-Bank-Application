...
export class TransactionsService {
  ...
  private baseURL = 'http://localhost:8086/transaction'

  constructor(private http: HttpClient) { }

  putTransaction(transaction: Transactions) : Observable<Message> {
    return this.http.post<Message>(`${this.baseURL}/addTransaction`, transaction);
  }

  getTransactions(accountNumber: string) : Observable<Array<Transactions>> {
    return this.http.get<any>(this.baseURL+"/transactions/"+accountNumber);
  }
