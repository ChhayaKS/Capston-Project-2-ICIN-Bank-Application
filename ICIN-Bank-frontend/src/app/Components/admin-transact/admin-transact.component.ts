...
export class AdminTransactComponent implements OnInit{
  ...
  ngOnInit(): void {
    // this.transactionService.getAllPendingTransaction().subscribe(transactions => {
...
getAll()
{
  this.transactionService.getAllPendingTransaction().subscribe(res=>
    {
      console.log(res);
      res.forEach(data=>
        {
  ...
  approveTransaction(id: number) {
    this.transactionService.permitTransaction(id) .subscribe(message => {
